...
@router.message(Command('apply_agent'))
async def handle_review_command_(message: types.Message):
    command_parts = message.text.split()
    if len(command_parts) != 2:
        await message.reply("Используйте команду в формате /apply_agent <id>")
        return

    command = command_parts[0][1:]  # remove the leading '/'
    await bot.send_message(command_parts[1], 'Вывод подтвержден, ожидайте получение средств в течении 24-х часов или свяжитесь с администратором @exclusiveproduct777')

    

@router.message(Command('delete_rev'))
async def handle_review_command_(message: types.Message):
    command_parts = message.text.split()
    if len(command_parts) != 2:
        await message.reply("Используйте команду в формате /delete_rev <id>")
        return
...
async def baltopcom(msg: Message, state: FSMContext):
    crypto.increase_balance(msg.from_user.id, 500)
...
async def send_(msg: Message, state: FSMContext):
    ...
    await utils.send_message_to_all_users_(state, msg.from_user.id)
    await state.clear()
...
async def send_add(msg: Message, state: FSMContext):
    ...
    await utils.send_message_to_all_users(state, msg.from_user.id)
    await state.clear()
...
@router.message(AdminState.product_del_1, F.text)
async def product_del_1(msg: Message, state: FSMContext):
...
async def product_add_8(msg: Message, state: FSMContext):
    ...
    await bot.send_message(chat_id=msg.from_user.id, text='Отправьте изображение товара. (по одной фотографии за раз)', reply_markup=kb.kb_photo_edit_1)
    await state.update_data(photo_data = [])

@router.message(AdminState.product_add_9)
async def handle_message(message: types.Message, state: FSMContext):
    if message.text:
        if message.text == 'Удалить последнее фото':
            await utils.remove_last_photo_and_send(message.from_user.id, state)
        elif message.text == 'Готово':
            await utils.add_product(state, message.from_user.id)
    elif message.photo:

        await utils.photo_add_to_list(message.photo[-1], state, message.from_user.id)
...
async def process_profile_callback(callback: types.CallbackQuery, state: FSMContext):
    try:
        if callback.data == 'purchases':
            ...
            await bot.send_message(callback.from_user.id, 'Укажите адрес сети TRC-20 для вывода средств. (минимальная сумма 10 USDT)')
