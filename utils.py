...
async def send_first_product_with_image(chat_id: int, data, state: FSMContext):
    ...
    if len(products) == 0:
        ...
        image_file = product[11]  # индекс изменен, чтобы отразить новую структуру
        ...
        await bot.send_photo(chat_id, image_file, caption=f"*{product[2]}*\n\n"
                                                f"*{product[5]} Руб*\n"
                                                f"*{product[6]} USDT*\n"
                                                f"*{product[7]} USDT На оффициальном сайте*\n\n"
                                                f"{product[3]}\n\n"
                                                f"{product[4]}\n\n"
                                                f"*{prod_8}\n\n*"
                                                f"Обычная доставка: {product[9]}\n"
                                                f"Экспресс доставка по Москве: {product[10]}\n\n"
                                                f"{num + 1} из {len(products)}", reply_markup=keyboard, parse_mode='Markdown')
...
async def send_message_to_all_users_(state):
    data = await state.get_data()
            ...
            if products:
                ...
                if product:
                    ...
                    if balance >= price_usdt:
                        ...
                        await bot.send_message(user_id, f"Успешная покупка, новый баланс составляет {new_balance} USDT.\nНе забудьте оставить отзыв!")
